...
impl Grid {
    ...
    fn bfs(&mut self) -> u32 {
        ...
        let mut border = vec![(portals["AA"][0], 0)];
        ...
        for travelled in 0.. {
            ...
            for (position, level) in border {
                ...
                for direction in Point::DIRECTIONS {
                    ...
                    if visited.contains(&(go, level)) {
                        continue;
                    } else if *entity == b'.' {
                        next.push((go, level));
                    } else if entity.is_ascii_alphabetic() {
                        let right = self.grid[0].len() as i16 - 2;
                        let bottom = self.grid.len() as i16 - 2;

                        let level = if go.x == 1 || go.x == right || go.y == 1 || go.y == bottom {
                            level - 1
                        } else {
                            level + 1
                        };
                        ...
                        if level >= 0 {
                            teleports.get(&position).map(|p| next.push((*p, level)));
                        }
                    }
                }
            }
