...
fn main() -> Result<()> {
    let mut computer = Computer::load("25.txt")?;

    loop {
        command(&mut computer)?;
    }
}

fn command(computer: &mut Computer) -> Result<()> {
    let mut input = String::new();
    io::stdin().read_line(&mut input)?;

    computer.line(&ascii(&input.trim()));
    computer.lines();
    Ok(())
}
...
impl Computer {
    ...
    fn lines(&mut self) -> Vec<String> {
        iter::from_fn(|| Some(self.line(&[])).filter(|s| s != "Command?" && !self.finished))
